...
Asana integration for Open Brain Phase 4. Read all tasks and projects. Write (create/update tasks, add comments, post status) only to explicitly approved projects with user approval gating.
Trigger phrases: "check my tasks", "asana tasks", "create a task", "update task status", "project status", "search asana".
Output artifact: task listings in conversation, task/comment/status objects in Asana via write tools (approved projects only).
