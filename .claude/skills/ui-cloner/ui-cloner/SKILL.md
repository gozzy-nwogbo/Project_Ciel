...
Pipeline Step 4 (entry point) of the UI Cloner (SRIP) pipeline. Entry skill that orchestrates the full 4-phase site replication workflow: forensic audit, brand interview, synthesis, and quality check. Asks for audit mode (Standard vs High-Fidelity) then delegates to each phase skill in sequence. Trigger: "clone this site", "replicate this UI", "ui cloner [URL]". Output artifact: `plans/` directory with all phase outputs.
