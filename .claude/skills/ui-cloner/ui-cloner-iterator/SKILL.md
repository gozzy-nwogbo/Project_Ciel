...
Pipeline Step 5 of 6 in the UI Cloner (SRIP) pipeline. Takes a poorly-executed first build attempt and runs 5 structured comparison passes against the Site DNA to systematically close fidelity gaps, producing targeted corrective prompts per pass. Trigger: "iterate on build", "run refinement loop", "compare against site DNA". Output artifact: `plans/05-iterator.md`.
