...
6. **Atom format:** YAML frontmatter per `.claude/standards/atom-front-matter.md` v2.0, lightweight atom pattern only. Required fields for concepts/frameworks/principles: `title`, `type` (one of: `concept`, `framework`, `principle`, `connection`), `source_date` (ISO 8601), `tags`. For connection atoms: `type: connection`, replace `tags` with `from` and `to` (plain human-readable titles matching the referenced atom's `title` field exactly — never wiki-links like `[[slug]]`, never kebab-case slugs). Body: 1-3 sentence single-claim, no section headings. Do not match existing folder formats — generate fresh per the standard regardless of what's already in the target folder.
